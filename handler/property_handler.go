...
func AddProperty(c *gin.Context) {
	...
	// ─── 1) VC 검증 ─────────────────────────────────────────────
    // DB에서 VC 꺼내기
    vc, err := database.GetBrokerVC(req.User)
    if err != nil {
        c.JSON(http.StatusUnauthorized, gin.H{"error": "공인중개사 인증이 필요합니다."})
        return
    }
    // 서명 재계산
    expectedSig, err := signVC(*vc)
    if err != nil {
        c.JSON(http.StatusInternalServerError, gin.H{"error": "VC 서명 검증 중 오류", "detail": err.Error()})
        return
    }
    if vc.Signature != expectedSig {
        c.JSON(http.StatusUnauthorized, gin.H{"error": "VC 검증에 실패했습니다."})
        return
    }
    // ──────────────────────────────────────────────────────────
